...
bwand2 (a,b) {
    ...
    while (i) {
        ...
        ret; asm("    rol() mov(to_ram, frm_acc)\n"); //  ret << 1
...
lbl("bwand")
    ...
    mov(to_acc, progdata) data(0x0)      // const
    pop(to_mar)
    mov(to_ram, frm_acc)
    ld(to_acc, SP)
    add(progdata) data(1)
    push_unsafe(frm_acc)              // 1st operand push
    mov(to_acc, progdata) data(0x8)      // const
    pop(to_mar)
    mov(to_ram, frm_acc)
lbl("__bwand_while_test")
    ld(to_acc, SP)
    add(progdata) data(1)
    mov(to_mar, frm_acc)
    mov(to_acc, frm_ram)
    jz("__bwand_while_end")
    dec() mov(to_ram, frm_acc)
    ld(to_acc, SP)
    add(progdata) data(2)
    mov(to_mar, frm_acc)
    mov(to_acc, frm_ram)
    rol() mov(to_ram, frm_acc)
    ld(to_acc, SP)
    add(progdata) data(6)
    mov(to_mar, frm_acc)
    mov(to_acc, frm_ram)
    ...
    add(frm_ram)
    push_unsafe(frm_acc)              // 1st operand push
    mov(to_acc, progdata) data(0xFE)      // const
    st(BX, frm_acc)            // store 2nd operand
    pop(to_acc)                // 1st operand pop
    mov(to_mar, progdata) data(BX)  // addition
    add(frm_ram)
...
lbl("__bwand_in")
    ...
    mov(to_mar, frm_acc)
    mov(to_acc, frm_ram)
    add(progdata) data(0x80)
lbl("__bwand_out")
    mov(to_ram, frm_acc)
    jp("__bwand_while_test")
lbl("__bwand_while_end")
    ld(to_acc, SP)
    add(progdata) data(2)
    mov(to_mar, frm_acc)
    mov(to_acc, frm_ram)
